...
export const Grid = styled.div<GridProps>`
    ...
    grid-template-columns: ${
        mobileColumnsCount === "auto"
            ? mobileColumnsCount
            : `repeat(${mobileColumnsCount}, ${mobileColumnsSize || "1fr"})`
    };
    grid-gap: ${mobileGridGap ?? 0};
  `}

    ${({ tabletColumnsCount, tabletGridGap, tabletColumnsSize }) => `
    ${tabletMedia(`
    grid-template-columns: ${
        tabletColumnsCount === "auto"
            ? tabletColumnsCount
            : `repeat(${tabletColumnsCount}, ${tabletColumnsSize || "1fr"})`
    };
      grid-gap: ${tabletGridGap ?? 0};
    `)}
  `}
  ...
  ${({ desktopColumnsCount, desktopGridGap, desktopColumnsSize }) => `
    ${desktopMedia(`
    grid-template-columns: ${
        desktopColumnsCount === "auto"
            ? desktopColumnsCount
            : `repeat(${desktopColumnsCount}, ${desktopColumnsSize || "1fr"})`
    };
      grid-gap: ${desktopGridGap ?? 0};
    `)}
  `}
`;
...
export const GridItem = memo(styled(Visible)<GridItemProps>`
    ...
    height: 100%;
    ${({ mobile }) =>
        mobile && `grid-column: ${mobile.start} / ${mobile.end + 1};`}
...
export const GridWithTemplate = styled(Grid)<GridWithTemplateProps>`
...
export const GridItemArea = styled(Visible)<GridItemAreaProps>`
    width: 100%;
    height: 100%;
